...
import { Copy, Plus, Trash2, Check, Key, Webhook, ToggleLeft, ToggleRight, Zap, ArrowRight } from 'lucide-react'
...
function ProGate() {
  ...
  return (
    <div style={{ maxWidth: 560, margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
      <div style={{ width: 64, height: 64, borderRadius: 16, background: '#fff1e6', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
        <Zap size={28} color="#dd5b00" />
      </div>
      <h1 style={{ fontSize: 26, fontWeight: 700, letterSpacing: -0.5, marginBottom: 12 }}>
        Integrations are a Pro feature
      </h1>
      <p style={{ fontSize: 15, color: 'var(--color-ink-muted)', lineHeight: 1.7, marginBottom: 32 }}>
        Upgrade to Pro to unlock REST API access, webhook integrations, and connect Noteify to tools like Zapier, n8n, and Make.
      </p>
      <div style={{ background: 'var(--color-canvas-soft)', border: '1px solid var(--color-hairline)', borderRadius: 12, padding: 24, marginBottom: 28, textAlign: 'left' }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 14, color: 'var(--color-ink-muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>What you get with Pro</div>
        {[
          'REST API access with API key management',
          'Webhook endpoints for real-time events',
          'Connect to Zapier, n8n, Make and more',
          'Unlimited file uploads',
          'Advanced CRM + Pipeline',
          'Priority support',
        ].map(f => (
          <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', fontSize: 14, color: 'var(--color-ink-secondary)' }}>
            <Check size={14} color="#1aae39" strokeWidth={2.5} /> {f}
          </div>
        ))}
      </div>
      <button className="btn-primary" onClick={() => navigate('/app/upgrade')}
        style={{ fontSize: 15, padding: '12px 28px', gap: 8 }}>
        Upgrade to Pro <ArrowRight size={15} />
      </button>
      <div style={{ marginTop: 12, fontSize: 13, color: 'var(--color-ink-faint)' }}>
...
export default function IntegrationsPage() {
  ...
  // Gate non-pro users
  if (profile?.plan !== 'pro') return <ProGate />
  ...
  async function createApiKey() {
    if (apiKeys.length >= 4) { alert("Maximum of 4 API keys allowed. Delete one to create a new one."); return }
  ...
  return (
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '40px 24px' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--color-ink-muted)' }}>Manage your API keys and webhook endpoints.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: 'var(--color-canvas-soft)', borderRadius: 8, padding: 4, width: 'fit-content' }}>
        {[{ id: 'api', label: 'API Keys', icon: <Key size={14} /> }, { id: 'webhooks', label: 'Webhooks', icon: <Webhook size={14} /> }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', fontSize: 14, fontWeight: 500,
            borderRadius: 6,
            ...
            cursor: 'pointer', boxShadow: tab === t.id ? 'var(--shadow-soft)' : 'none',
          }}>
            {t.icon} {t.label}
          </button>
        ))}
        ...
        <div>
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-hairline)', borderRadius: 12, padding: 24, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <input style={{ ...inputStyle, flex: 1 }} placeholder="Key name e.g. n8n integration"
                value={newKeyName} onChange={e => setNewKeyName(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && createApiKey()} />
              <button onClick={createApiKey} disabled={loadingKey || !newKeyName.trim()} style={{
                display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', fontSize: 14, fontWeight: 500,
                background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 9999,
                cursor: 'pointer', opacity: !newKeyName.trim() ? 0.5 : 1, whiteSpace: 'nowrap',
              }}>
              ...
              <div style={{ marginTop: 16, padding: '12px 16px', background: '#f0fff4', border: '1px solid #bbf7d0', borderRadius: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <button onClick={() => copyToClipboard(createdKey, 'new')} style={{
                    display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', fontSize: 13,
                    borderRadius: 6, border: '1px solid #bbf7d0', background: '#fff', cursor: 'pointer', color: '#166534', whiteSpace: 'nowrap',
                  }}>
          ...
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-hairline)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--color-hairline)' }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><h2 style={{ fontSize: 16, fontWeight: 600 }}>Your API keys</h2><span style={{ fontSize: 12, color: apiKeys.length >= 4 ? "#c0392b" : "var(--color-ink-faint)", fontWeight: 500 }}>{apiKeys.length}/4 keys used</span></div>
            </div>
            {apiKeys.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-ink-faint)', fontSize: 14 }}>No API keys yet. Create one above.</div>
            ) : apiKeys.map((k, i) => (
              <div key={k.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', borderBottom: i < apiKeys.length - 1 ? '1px solid var(--color-hairline)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--color-ink-faint)', marginTop: 2 }}>
                    Created {new Date(k.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })} · {k.last_used_at ? `Last used ${new Date(k.last_used_at).toLocaleDateString()}` : 'Never used'}
                  </div>
                </div>
                <button onClick={() => deleteApiKey(k.id)} style={{ padding: 7, borderRadius: 6, border: '1px solid var(--color-hairline)', background: 'var(--color-surface)', color: '#c0392b', cursor: 'pointer', display: 'flex' }}>
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
      ...
      {tab === 'webhooks' && (
        <div>
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-hairline)', borderRadius: 12, padding: 24, marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: showWebhookForm ? 20 : 0 }}>
              ...
              <button onClick={() => setShowWebhookForm(f => !f)} style={{
                ...
                border: '1px solid var(--color-hairline)', borderRadius: 9999, cursor: 'pointer',
              }}>
            ...
            {showWebhookForm && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <button onClick={createWebhook} disabled={loadingWebhook || !webhookName.trim() || !webhookUrl.trim() || selectedEvents.length === 0} style={{
                  padding: '10px 20px', fontSize: 14, fontWeight: 500,
                  background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 9999, cursor: 'pointer',
                  opacity: (!webhookName.trim() || !webhookUrl.trim() || selectedEvents.length === 0) ? 0.5 : 1, alignSelf: 'flex-start',
                }}>
          ...
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-hairline)', borderRadius: 12, overflow: 'hidden' }}>
            {webhooks.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-ink-faint)', fontSize: 14 }}>No webhooks yet. Add one above to start receiving events.</div>
            ) : webhooks.map((w, i) => (
              <div key={w.id} style={{ padding: '16px 20px', borderBottom: i < webhooks.length - 1 ? '1px solid var(--color-hairline)' : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 9999, background: w.active ? '#dcfce7' : 'var(--color-canvas-soft)', color: w.active ? '#166534' : 'var(--color-ink-faint)' }}>
                        {w.active ? 'Active' : 'Paused'}
                      </span>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--color-ink-faint)', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{w.url}</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                      {w.events.map(e => <span key={e} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 9999, background: 'var(--color-canvas-soft)', color: 'var(--color-ink-muted)', border: '1px solid var(--color-hairline)' }}>{e}</span>)}
                    </div>
                  </div>
